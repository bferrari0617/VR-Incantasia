...
public class Spellbook : MonoBehaviour
{
    ...
    private const OVRInput.RawButton ThumbstickButton = OVRInput.RawButton.LThumbstick; // Corrected constant


    // Start is called before the first frame update
    void Start()
    {
        VRIsActive = OVRPlugin.productName == "Oculus"; // Check if Oculus HMD is present
        camera = Camera.main;
    ...
    float GetInputValue()
    {
        if (VRIsActive)
        {
            Vector2 touchpadPos = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick, OVRInput.Controller.LTouch);
            return touchpadPos.x * pageTurnRate;
        }
        else
        {
    ...
    bool IsInputActive()
    {
        if (VRIsActive)
        {
            return OVRInput.Get(ThumbstickButton, OVRInput.Controller.LTouch);
        }
        else
        {
